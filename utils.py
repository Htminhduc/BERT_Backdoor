...
#     reference_model.eval()
#     for param in reference_model.parameters():
#         param.requires_grad = False  # Ensure reference model is frozen

#     # Forward pass (reference model)
#     with torch.no_grad():  # Disable gradient computation for reference model
#         ref_enc_output = reference_model.encoder(input_ids, attention_mask)
#         ref_channel_enc_output = reference_model.channel_encoder(ref_enc_output)
#         ref_Tx_sig = PowerNormalize(ref_channel_enc_output)
...
#         ref_channel_dec_output = reference_model.channel_decoder(ref_Rx_sig)
#         ref_pred_logits = reference_model.decoder(ref_channel_dec_output, ref_channel_dec_output)
#         ref_pred_logits = ref_pred_logits.detach()  # Detach from computation graph
...
#     poison_channel_dec_output = poison_model.channel_decoder(poison_Rx_sig)
#     poison_pred_logits = poison_model.decoder(poison_channel_dec_output, poison_channel_dec_output)
#     poison_loss = criterion(poison_pred_logits, trg)

#     # Compute divergence loss
#     divergence_loss = loss1(poison_pred_logits.clone(), ref_pred_logits.clone())

#     # Total loss for poisoned model
#     total_poison_loss = poison_loss + beta * divergence_loss

#     # Optimize poisoned model
#     opt_poison.zero_grad()
#     total_poison_loss.backward()
#     torch.nn.utils.clip_grad_norm_(poison_model.parameters(), max_norm=1.0)
#     opt_poison.step()

#     return poison_loss.item(), divergence_loss.item()
def train_step_with_smart_simple_JSCC(model, trg, opt, criterion,
                                 ...
                                 lambda_rate=0.01):
    model.train()
    # --- 1) forward + rate from hyperprior ---
...
def val_step_with_smart_simple_JSCC(model, trg, criterion,
    ...
    with torch.no_grad():
        ...
        pred_logits, rate_loss = model(input_ids, attention_mask, channel, n_var)
