...
parser.add_argument('--checkpoint-path', default='/home/necphy/ducjunior/BERT_Backdoor/checkpoints/deepsc_AWGN_JSSC_type2', type=str)
...
if __name__ == '__main__':
    ...
    for name, param in deepsc.named_parameters():
        ...
        if "hyper_encoder" not in name and "hyper_decoder" not in name and "lastlayer" not in name and "decoder" not in name and "channel_decoder" not in name:
    ...
    optimizer = torch.optim.AdamW(deepsc.parameters(),
                                 lr=10e-5, betas=(0.9, 0.98), eps=1e-8, weight_decay=1e-5)
