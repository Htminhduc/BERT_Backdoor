...
import numpy as np
from utils import SNR_to_noise, initialize_weights, train_step_with_smart_simple_JSCC_MOD,val_step_with_smart_simple_JSCC
...
def validate(epoch, args, net):    
    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            noise_val = np.random.uniform(SNR_to_noise(0), SNR_to_noise(10))
            n_var = torch.full((bs,),
                       noise_val,
                       device=device,
                       dtype=torch.float)
...
# def train(epoch, args, net,criterion, opt, mi_net=None):
    ...
    for batch in pbar:
        ...
        pbar.set_description(f'Epoch: {epoch + 1}; Type: Train; Loss: {loss:.5f},ori_loss: {ori_loss:.5f},smooth_loss: {smooth_loss:.5f},rate_loss: {rate_loss:.5f}, mod_loss: {mod_loss:.5f}')
