...
def inspect_sentence_clean(model, sentence, channel="AWGN", n_var=0.0, device="cuda:0"):
    model.eval()
...
# def compute_bits(model, tokenizer, sentence, channel='AWGN', n_var=0.0, device='cpu'):
#     model.eval().to(device)
#     with torch.no_grad():
#         enc = tokenizer(sentence, return_tensors='pt', padding=True).to(device)
#         seq_emb = model.encoder(enc.input_ids, enc.attention_mask)  # [1, T, D]
#         y       = seq_emb[:, 0, :]                                  # [1, D]

#         z       = model.hyper_encoder(y)
#         z_tilde = torch.round(z)
#         params  = model.hyper_decoder(z_tilde)
#         mu, raw_s = params.chunk(2, dim=1)
#         sigma = F.softplus(raw_s)

#         y_tilde = torch.round(y)

#         iv = 1.0/(sigma*math.sqrt(2))
#         p_y = 0.5*(torch.erf((y_tilde+0.5-mu)*iv) - torch.erf((y_tilde-0.5-mu)*iv)).clamp(min=1e-12)
#         p_z = 0.5*(torch.erf((z_tilde+0.5)*iv) - torch.erf((z_tilde-0.5)*iv)).clamp(min=1e-12)

#         B_y = (-torch.log2(p_y).sum()).item()
#         B_z = (-torch.log2(p_z).sum()).item()
#         bits = B_y + B_z

#         logits, _ = model(enc.input_ids, enc.attention_mask, channel, n_var)
...
#         rows.append({
#             'sentence': sentence,
#             'bits': *compute_bits(deepsc, tokenizer, sentence, 'AWGN', 0.0, args.device)
#         })
#         if len(rows) >= 5:
#             break
#     if len(rows) >= 5:
...
# df = pd.DataFrame(rows, columns=['sentence','bits','prediction'])
# display_dataframe_to_user("Adaptive Rate Sample", df)
